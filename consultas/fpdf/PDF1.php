...
class pdf extends FPDF
{
   ...
   public function setData(array $data)
   {
      ...
      $this->Estatus = $this->resultadoDatos['ESTATUS'];
...
$consulta_estatus = "SELECT TIPO_BAJA, FECHA, OBSERVACIONES FROM Bajas WHERE CURP = '" .$fpdf->curp. "'";
$estatus = $connection->prepare($consulta_estatus);
if($estatus->execute()){
  $row = $estatus->fetch();
  if ($row) {
      $stat=$row['TIPO_BAJA'];
 //     echo "TIPO DE BAJA: ".$row['TIPO_BAJA'] . "<br> " ." FECHA DE BAJA: ". $row['FECHA']."<br>"."OBSERVACIONES: ". $row['OBSERVACIONES'];
  } else {
      $stat='Activo';
  }
} else {
...
if($estatus->execute()){
  ...
  if ($row) {
      ...
      $fpdf->Cell(0,10,"Estatus de personal:{$stat} ",0,0);
      $fpdf->Ln();
      $fpdf->Cell(0,10,"Fecha de baja:{$dateb} ",0,0);
      $fpdf->Ln();
      $fpdf->Cell(0,10,"Observaciones:{$observation} ",0,0);
      $fpdf->Ln();
 } else {
      $stat='ACTIVO';
      $fpdf->Cell(0,10,"Estatus de personal:{$stat} ",0,0);
...
$fpdf->Image(__DIR__ . '/SICAH_marca_agua.png',10,50,200,200,'png');
